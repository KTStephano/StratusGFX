...
namespace stratus { 
    ...
    private:
        ...
        virtual SystemStatus Update(const f64);
        virtual void Shutdown();

    private:
        template<typename E, typename T>
        Async<E> CreateAsyncTask_(const T& process, const usize index) {
        ...
        Async<void> CreateAsyncVoidTask_(const std::function<void (void)>& process, const usize index) {
            // Increment the working #
            threadsWorking_[index]->fetch_add(1);
        ...
        usize GetNextThreadIndexForTask_() {
            ...
            std::vector<std::pair<ThreadHandle, usize>> currentWorkLoads;
            ...
            const auto comparison = [](const std::pair<ThreadHandle, usize>& a, const std::pair<ThreadHandle, usize>& b) {
