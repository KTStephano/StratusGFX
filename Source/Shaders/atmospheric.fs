...
uniform vec3 normalizedCameraLightDirection; // This is in camera space and is the direction to the light source (see page 152, eq. 8.21)
...
float sampleShadowTexture(vec4 coords, float cascadeSwitch, float bias) {
    return sampleShadowTextureSparse1Sample(infiniteLightShadowMap, infiniteLightShadowMapNonFiltered, coords.xyz, vec2(0.0), bias, int(cascadeSwitch), true);
}
// float sampleShadowTexture(vec4 coords, float cascadeSwitch, float bias) {
