...
void main() {
    ...
    for (int i = 0; i < numShadowLights; ++i) {
        ...
        if(distance < light.radius) {
            ...
            if (viewDist < 150.0) {
                shadowFactor = calculateShadowValue8Samples(shadowCubeMaps[entry.index], entry.layer, light.farPlane, fragPos, light.position.xyz, dot(light.position.xyz - fragPos, normal), 0.02);
            }
            else if (viewDist < 500.0) {
                shadowFactor = calculateShadowValue1Sample(shadowCubeMaps[entry.index], entry.layer, light.farPlane, fragPos, light.position.xyz, dot(light.position.xyz - fragPos, normal), 0.02);
            }
