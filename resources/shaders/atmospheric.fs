...
uniform vec3 shadowSpaceCameraPos;           // M_shadow(0) * M_camera[3]
...
void calculateShadowSpaceMinMaxRayPoints(vec2 camSpaceRayDir, vec3 shadowSpaceRayDir, out float invLength, out float scale, out vec3 p1, out vec3 p2) {
    invLength = 1.0 / length(vec3(camSpaceRayDir, minAtmosphereDepth));
    scale = minAtmosphereDepth * invLength;

    p1 = scale * shadowSpaceRayDir;
...
float sampleShadowTexture(sampler2DArrayShadow shadow, vec4 coords, float bias) {
    //vec3 sampleCoords = coords.xyz / coords.w; // Perspective divide
    //sampleCoords = sampleCoords * 0.5 + vec3(0.5); // Convert to range [0, 1]

    return texture(shadow, coords.xyz, coords.w - bias);
}
...
float calculateFinalBrightness(vec2 pixelCoords, float z1, float z2, vec3 p1, vec3 p2, float anisotropicScatteringIntensity) {
    ...
    float structure = texture(structureBuffer, pixelCoords).zw;
    ...
    for (; t <= tmax; t += deltaT) {
        ...
        sampleCoords.xyw = lerp(p1, p2, u);
        atmosphere += weight * sampleShadowTexture(infiniteLightShadowMap, sampleCoords, bias);
    }
