...
void main() {
    ...
    for (int i = 0; i < numShadowLights; ++i) {
        ...
        color = color + calculatePointLighting(baseColor, normal, viewDir, i, roughness, 1.0, shadowFactor);
    }

    for (int i = numShadowLights; i < numLights; ++i) {
        //if (i >= numLights) break;
        color = color + calculatePointLighting(baseColor, normal, viewDir, i, roughness, 1.0, 0.0);
    }
