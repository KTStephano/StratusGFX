...
float normalDistribution(const float NdotH) {
    float roughnessSquared = fsRoughness * fsRoughness;
...
float geometry(vec3 normal, vec3 viewDir, vec3 lightDir) {
    float k = pow(fsRoughness + 1, 2) / 8.0;
