...
void main() {
    ...
    for (int i = 0; i < numShadowLights; ++i) {
        ...
        color = color + calculatePointLighting(baseColor, normal, viewDir, i, roughness, ao, shadowFactor);
    }
    for (int i = numShadowLights; i < numLights; ++i) {
        //if (i >= numLights) break;
        color = color + calculatePointLighting(baseColor, normal, viewDir, i, roughness, ao, 0.0);
    }
    color = color + baseColor * ao * ambientIntensity;
